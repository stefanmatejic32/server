...
function findMatchingTimeslot(timeslots, year, month, day, hour, minute) {
  ...
  const foundTimeslot = timeslots.find(function (element) {
    ...
    return element.startTime.includes(hour - 1 + ":" + minute + ":00");
  });
 ...
 */
function bookAppointment(auth, year, month, day, hour, minute, user) {
  return new Promise(function (resolve, reject) {
    ...
    var ts1 = timeslot.time.startTime;
    //  ts1.setHours(ts1.getHours() - 1);

    console.log(timeslot.time.startTime);
    var ts2 = timeslot.time.endTime;

    const event = appUtil.makeEventResource(
      date,
    ...
    calendar.events.insert(
      ...
      function (err, res) {
        if (err)
          return console.log("Error contacting the Calendar service: " + err);
